      ...
      if (gmailEmail) {
        try {
          const dbUser = await db
            .select({ id: user.id })
            .from(user)
            .where(eq(user.email, gmailEmail))
            .limit(1)
            .then(rows => rows[0]);

          if (dbUser) {
            activeTenantId = dbUser.id;
            ...
            const [local, domain] = gmailEmail.split('@');
            const maskedLocal = local ? (local.length > 2 ? `${local[0]}${'*'.repeat(local.length - 2)}${local[local.length - 1]}` : `${local[0]}*`) : '***';
            logger.error(`[Webhook POST] No user found in database for email: ${maskedLocal}@${domain || 'unknown'}`);
            // Acknowledge the Pub/Sub message to prevent retries for non-existent users
            return new Response(JSON.stringify({ success: true, message: 'No user found in database' }), {
              status: 200,
              headers: { 'Content-Type': 'application/json' },
            });
          }
        } catch (err) {
          logger.error('[Webhook POST] User lookup failed:', err);
        }
      } else {
